...
// 캐싱
let _fileTypeFromBuffer: null | ((buf: Buffer) => Promise<any>) = null;

/**
 * file-type 로더
 * - 테스트 환경 우회 및 ESM-safe dynamic import 처리
 */
...
export const assertAllowedByMagic = async (buffer: Buffer) => {
  ...
  if (!detected) {
    throw ApiError.badRequest('파일 형식을 인식할 수 없습니다.');
  }

  const mime = detected.mime as string;
  logger.system.debug(`[File] 업로드 Mime: ${mime}`);

  if (!(ALLOWED_IMAGE_MIMES as readonly string[]).includes(mime)) {
    throw ApiError.badRequest(`허용되지 않은 이미지 형식입니다. (${mime})`);
  }
